class Harvest < ApplicationRecord
  ...
  scope :one_per_owner,
        lambda {
          joins(
            'JOIN members m ON (m.id=harvests.owner_id)
           LEFT OUTER JOIN harvests h2
           ON (m.id=h2.owner_id AND harvests.id < h2.id)'
          )
            .where('h2 IS NULL')
        }
  ...
  validates :quantity, allow_nil: true, numericality: { only_integer: false, greater_than_or_equal_to: 0 }
  validates :unit, allow_blank: true, inclusion: { in: UNITS_VALUES.values, message: '%<value>s is not a valid unit' }
  validates :weight_quantity, allow_nil: true, numericality: { only_integer: false }
  validates :weight_unit,
            allow_blank: true, inclusion: { in: WEIGHT_UNITS_VALUES.values, message: '%<value>s is not a valid unit' }
