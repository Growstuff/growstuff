...
module SearchPlantings
  ...
  included do
    ...
    def search_data
      {
        ...
        thumbnail_url:
                                    # photos
                                    default_photo&.thumbnail_url || crop.default_photo&.thumbnail_url,
        # counts
    ...
    def self.homepage_records(limit)
      ...
      owners = []
      1..limit.times do
        where = { photos_count: { gt: 0 }, owner_id: { not: owners } }
        one_record = search('*', limit: 1, where: where, boost_by: [:created_at], load: false).first
