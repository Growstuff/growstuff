...
class Planting < ApplicationRecord
  ...
  scope :located,
        lambda {
          joins(:garden).where.not(gardens: { location: '' }).where.not(gardens: { latitude: nil }).where.not(
            gardens: { longitude: nil }
          )
        }
  ...
  scope :one_per_owner,
        lambda {
          joins(
            'JOIN members m ON (m.id=plantings.owner_id)
           LEFT OUTER JOIN plantings p2
           ON (m.id=p2.owner_id AND plantings.id < p2.id)'
          ).where('p2 IS NULL')
        }
  ...
  def nearby_same_crop
    ...
    Planting.joins(:garden).where(crop: crop).located.where(
      'gardens.latitude < ? AND gardens.latitude > ?',
      latitude + 10,
      latitude - 10
    )
  end
