...
class CropSearchService
  ...
  def self.random_with_photos(limit)
    body = {
      "query": {
        "function_score": {
          "query": { "query_string": { "query": 'has_photos:true' } }, "random_score": { "seed": DateTime.now.to_i }
        }
      }
    }
    Crop.search(limit: limit, load: false, body: body)
  end

  def self.recent(limit)
    Crop.search(limit: limit, load: false, boost_by: { created_at: { factor: 100 } }) # default factor is 1
  end
end
