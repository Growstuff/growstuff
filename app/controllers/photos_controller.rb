class PhotosController < ApplicationController
  ...
  def index
    ...
    @photos = @photos.order(created_at: :desc).includes(:owner).paginate(page: params[:page])
    respond_with(@photos)
  end
  ...
  def retrieve_from_flickr
    ...
    @photos = WillPaginate::Collection.create(page, 30, total) { |pager| pager.replace photos }
  end
end
