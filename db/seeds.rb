...
def load_crops
  ...
  Dir.glob("#{source_path}/crops*.csv").each do |crop_file|
    ...
    CSV.foreach(crop_file) { |row| CsvImporter.new.import_crop(row) }
  end
...
def load_test_users
  ...
  (1..member_size).each do |i|
    @user =
      Member.new(login_name: "test#{i}", email: "test#{i}@example.com", password: "password#{i}", tos_agreement: true)
...
def load_admin_users
  ...
  @admin_user =
    Member.new(login_name: 'admin1', email: 'admin1@example.com', password: 'password1', tos_agreement: true)
  @admin_user.skip_confirmation!
  @admin_user.roles << @admin
  @admin_user.save!

  @wrangler_user =
    Member.new(login_name: 'wrangler1', email: 'wrangler1@example.com', password: 'password1', tos_agreement: true)
...
load_data # rubocop:disable Metrics/AbcSize
