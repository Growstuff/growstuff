...
[apple](crop)
  ...
  before do
    ...
    Timecop.freeze(Time.zone.local(2_019, 1, 1))
  ...
  shared_examples 'visit pages' do
    ...
    describe 'posts' do
      ...
      it 'loads posts#index' do
        Member.all.limit(5).each { |member| FactoryBot.create_list :post, 12, author: member }
        Post.all.order(id: :desc).limit(4) { |post| FactoryBot.create_list :comment, rand(1..5), post: post }
        visit posts_path
