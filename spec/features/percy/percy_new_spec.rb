...
[apple](crop)
  ...
  shared_examples 'visit pages' do
    ...
    describe 'crops' do
      it 'loads crops#show' do
        ...
        tomato.posts << FactoryBot.create(:post, subject: 'tomatoes are delicious')
    ...
    describe 'gardens' do
      let(:first_garden) { Garden.first }
      let(:second_garden) { Garden.second }
      let(:third_garden) { Garden.all.offset(3)[0] }
      it 'loads gardens#index' do
        FactoryBot.create_list :planting, 3, garden: first_garden, owner: first_garden.owner
        FactoryBot.create_list :planting, 7, garden: second_garden, owner: second_garden.owner
        FactoryBot.create_list :planting, 26, garden: third_garden, owner: third_garden.owner
        visit gardens_path
  ...
  context 'when signed in' do
    ...
    let(:my_first_garden) { Garden.first }
    let(:my_second_garden) { Garden.second }
    let(:my_third_garden) { Garden.all.offset(3)[0] }

    it 'loads gardens#index' do
      FactoryBot.create_list :planting, 3, garden: my_first_garden, owner: my_first_garden.owner
      FactoryBot.create_list :planting, 7, garden: my_second_garden, owner: my_second_garden.owner
      FactoryBot.create_list :planting, 26, garden: my_third_garden, owner: my_third_garden.owner
      visit gardens_path
      Percy.snapshot(page, name: "#{prefix}/gardens#index")
    end
    ...
    describe '#new' do
      it 'plantings#new' do
        ...
        fill_in 'crop', with: 'cap'
        Percy.snapshot(page, name: "#{prefix}/crops-autocomplete")
      end
