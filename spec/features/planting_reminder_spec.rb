...
describe 'Planting reminder email', :js do
  ...
  context 'when member has some plantings' do
    ...
    let!(:p1) do
      FactoryBot.create :predicatable_planting, planted_at: 10.days.ago, garden: member.gardens.first, owner: member
    end
    let!(:p2) do
      FactoryBot.create :predicatable_planting, planted_at: 30.days.ago, garden: member.gardens.first, owner: member
    end
  ...
  context 'when member has some harvests' do
    ...
    let!(:p1) do
      FactoryBot.create :predicatable_planting, garden: member.gardens.first, owner: member, planted_at: 20.days.ago
    end
