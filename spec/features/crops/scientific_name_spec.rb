...
describe 'Scientific names', js: true do
  ...
  context 'User is a crop wrangler' do
    ...
    it 'Crop wranglers can delete scientific names' do
      visit crop_path(zea_mays.crop)
      click_link zea_mays.name
      expect(page).to have_link 'Delete', href: scientific_name_path(zea_mays)
      within('.scientific_names') { accept_confirm { click_link 'Delete' } }
      # expect(page.status_code).to equal 200
    ...
    it 'Crop wranglers can add scientific names' do
      visit crop_path(crop)
      expect(page).to have_link 'Add', href: new_scientific_name_path(crop_id: crop.id)
    ...
    it 'The show-scientific-name page works' do
      ...
      # expect(page.status_code).to equal 200
      expect(page).to have_link zea_mays.crop.name, href: crop_path(zea_mays.crop)
    end
