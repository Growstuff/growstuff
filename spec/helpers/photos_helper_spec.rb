...
describe PhotosHelper do
  ...
  let(:garden_photo)   { FactoryBot.create(:photo, thumbnail_url: 'https://example.com/garden.jpg', owner: garden.owner)   }
  let(:planting)       { FactoryBot.create :planting, crop: crop, owner: garden.owner                  }
  let(:planting_photo) { FactoryBot.create(:photo, thumbnail_url: 'https://example.com/planting.jpg', owner: garden.owner) }
  let(:harvest)        { FactoryBot.create :harvest, crop: crop, owner: garden.owner                   }
  let(:harvest_photo)  { FactoryBot.create(:photo, thumbnail_url: 'https://example.com/harvest.jpg', owner: garden.owner)  }
  let(:seed)           { FactoryBot.create :seed, crop: crop, owner: garden.owner                      }
  let(:seed_photo)     { FactoryBot.create(:photo, thumbnail_url: 'https://example.com/seed.jpg', owner: garden.owner)     }
