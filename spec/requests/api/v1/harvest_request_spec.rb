...
RSpec.describe 'Harvests', type: :request do
  ...
  let(:crop_as_json_api) do
    { 'links' => { 'self' => "#{resource_url}/relationships/crop", 'related' => "#{resource_url}/crop" } }
  end

  let(:owner_as_json_api) do
    { 'links' => { 'self' => "#{resource_url}/relationships/owner", 'related' => "#{resource_url}/owner" } }
  end

  let(:planting_as_json_api) do
    { 'links' => { 'self' => "#{resource_url}/relationships/planting", 'related' => "#{resource_url}/planting" } }
  end
  ...
  it '#create' do
    expect {
      put '/api/v1/harvests', headers: headers, params: { 'harvest' => { 'description' => 'can i make this' } }
    }.to raise_error ActionController::RoutingError
  end

  it '#update' do
    expect {
      post "/api/v1/harvests/#{harvest.id}",
           headers: headers, params: { 'harvest' => { 'description' => 'can i modify this' } }
    }.to raise_error ActionController::RoutingError
  end
