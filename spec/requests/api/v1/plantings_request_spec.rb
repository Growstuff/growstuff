...
RSpec.describe 'Plantings', type: :request do
  ...
  let(:garden_as_json_api) do
    { 'links' => { 'self' => "#{resource_url}/relationships/garden", 'related' => "#{resource_url}/garden" } }
  end
  ...
  it '#update' do
    expect do
      post "/api/v1/plantings/#{planting.id}",
           headers: headers, params: { 'planting' => { 'description' => 'can i modify this' } }
    end.to raise_error ActionController::RoutingError
  end
