...
RSpec.describe 'Members', type: :request do
  ...
  let(:harvests_as_json_api) do
    { 'links' => { 'self' => "#{resource_url}/relationships/harvests", 'related' => "#{resource_url}/harvests" } }
  end

  let(:photos_as_json_api) do
    { 'links' => { 'self' => "#{resource_url}/relationships/photos", 'related' => "#{resource_url}/photos" } }
  end

  let(:seeds_as_json_api) do
    { 'links' => { 'self' => "#{resource_url}/relationships/seeds", 'related' => "#{resource_url}/seeds" } }
  end

  let(:plantings_as_json_api) do
    { 'links' => { 'self' => "#{resource_url}/relationships/plantings", 'related' => "#{resource_url}/plantings" } }
  end
  let(:gardens_as_json_api) do
    { 'links' => { 'self' => "#{resource_url}/relationships/gardens", 'related' => "#{resource_url}/gardens" } }
  end

  let(:attributes) { { 'login-name' => member.login_name, 'slug' => member.slug } }
  ...
  it '#update' do
    expect {
      post "/api/v1/members/#{member.id}",
           params: { 'member' => { 'login_name' => 'can i modify this' } }, headers: headers
    }.to raise_error ActionController::RoutingError
  end
