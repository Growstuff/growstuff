...
describe Follow do
  ...
  it 'sends a notification when a follow is created' do
    expect { Follow.create(follower_id: @member1.id, followed_id: @member2.id) }.to change(Notification, :count).by(1)
  end
  ...
  context 'when follow is created' do
    before { @follow = Follow.create(follower_id: @member1.id, followed_id: @member2.id) }
