...
describe Post do
  ...
  context 'recent activity' do
    before { Time.stub(now: Time.zone.now) }
    ...
    it 'sets recent activity to comment time' do
      comment = FactoryBot.create(:comment, post: post, created_at: 1.hour.ago)
  ...
  context 'notifications' do
    ...
    it 'sends a notification when a member is mentioned using @-syntax' do
      expect { FactoryBot.create(:post, author: member, body: "Hey @#{member2}") }.to change(Notification, :count).by(1)
    end

    it 'sends a notification when a member is mentioned using [](member) syntax' do
      expect { FactoryBot.create(:post, author: member, body: "Hey [#{member2}](member)") }.to change(
        Notification,
        :count
      )
        .by(1)
    ...
    it 'sends notifications to all members mentioned' do
      ...
      expect { FactoryBot.create(:post, author: member, body: "Hey @#{member2} & @#{member3}") }.to change(
        Notification,
        :count
      )
        .by(2)
    ...
    it "doesn't send notifications if you mention yourself" do
      expect { FactoryBot.create(:post, author: member, body: "@#{member}") }.to change(Notification, :count).by(0)
    end
  end
